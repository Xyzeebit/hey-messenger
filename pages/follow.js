...
export default function FunctionalPage() {
  ...
  useEffect(() => {
    let indexOfQ = window.location.href.indexOf("?");
    if (indexOfQ > -1) {
      setULink(router.asPath.split("=")[1]);
    } else {
      router.push('/login');
    }
  }, []);
  ...
  useEffect(() => {
    const getUserWithLink = async (uLink) => {
        ...
        if (user.successful) {
          if (localStorage.getItem("hey_messenger")) {
            const localSession = JSON.parse(
              localStorage.getItem("hey_messenger")
            );
            setUserInfo({
              ...user,
  ...
  return (
    <div className="user_details">
      <div className="user_details__image--container">
        <img src={`uploads/${imageUrl}`} alt={username} width={100} height={100} />
      </div>
...
const FollowUser = ({ userInfo, onEvent }) => {
  return (
    <div className="follow_user">
      ...
      <hr />
      {!userInfo.added && (
        <UserActions isLoggedIn={userInfo.isLoggedIn} onEvent={onEvent} />
      )}
